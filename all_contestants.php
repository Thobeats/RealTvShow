...
if($step == "view_profile"){
    ...
    $writer = mysqli_fetch_assoc(mysqli_query($link, "Select * from realtv_contestants where unique_id = '$profileId'"));
}
...
<section class="profile-body p-2 mx-2">
        ...
        <div class="nav nav-tabs text-center mx-auto" id="v-pills-tab" role="tablist">
            <a class="nav-link <?= $step == 'all' ? 'tab-active' : '' ?>" href="?step=all">All Contestants</a>
            <?php if($step == 'view_profile'): ?>
            <a class="nav-link <?= $step == 'view_profile' ? 'tab-active' : '' ?>" href="?step=resume">Contestant's Profile</a>
            <?php endif; ?>
        </div>
